...
try:
    ...
    CPP_PROMPTS_AVAILABLE = True
except ImportError:
    CPP_PROMPTS_AVAILABLE = False

try:
    from templates.ruby_prompts import build_ruby_coding_user_prompt
    RUBY_PROMPTS_AVAILABLE = True
except ImportError:
    RUBY_PROMPTS_AVAILABLE = False
...
LANGUAGE_DEFAULTS = {
    ...
    "cpp": ("solution.cpp", "test_solution.cpp"),
    "c++": ("solution.cpp", "test_solution.cpp"),
    "ruby": ("solution.rb", "test_solution.rb"),
}
...
class CodeGenerator:
    ...
    def generate(
        ...
        # Use language-specific prompts when available
        if language in ["cpp", "c++"] and CPP_PROMPTS_AVAILABLE:
            base_user_prompt = build_cpp_coding_user_prompt(
                problem=problem,
                classification=classification,
                # Use plan length as complexity proxy
                complexity=len(plan_steps),
                understanding=understanding,
                plan=plan_steps,
                language=language,
            )
        elif language == "ruby" and RUBY_PROMPTS_AVAILABLE:
            base_user_prompt = build_ruby_coding_user_prompt(
                problem=problem,
                classification=classification,
                # Use plan length as complexity proxy
                complexity=len(plan_steps),
