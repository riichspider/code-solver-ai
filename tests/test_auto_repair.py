...
class TestAutoRepairManager:
    ...
    def setup_method(self):
        """Setup para cada teste."""
        self.mock_strategy1 = MockRepairStrategy(
            "strategy1", can_repair_result=True)
        self.mock_strategy2 = MockRepairStrategy(
            "strategy2", can_repair_result=False)
        self.manager = AutoRepairManager(
            [self.mock_strategy1, self.mock_strategy2])
...
class TestFactoryFunction:
    ...
    def test_create_with_ai_client(self):
        ...
        assert any(isinstance(s, PatternBasedRepairStrategy)
                   for s in manager.strategies)
        assert any(isinstance(s, PromptBasedRepairStrategy)
                   for s in manager.strategies)
